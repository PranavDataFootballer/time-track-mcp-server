...
def get_connection():
    # SQLite needs the DIRECTORY containing the DB to be writable,
    ...
    # Helps when the website and MCP server access SQLite at the same time.
...
def log_time(
    ...
    try:
        ...
        raise ValueError(
            "entry_date must be in YYYY-MM-DD format, e.g. 2026-09-21"
        )
...
def execute_query(query: str, params: list | None = None) -> list[dict]:
    conn = get_connection()
    rows = conn.execute(query, params).fetchall()
    conn.close()
    return [_row_to_dict(r) for r in rows]
